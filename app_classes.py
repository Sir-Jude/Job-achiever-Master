...
class Adviser_Bot:
    def __init__(self, candidate, recrutier, job):
        ...
        # Empty roles for bot init
        self.role_description =''
        self.user_input =''

    def generate_letter(self):
        self.role_description = """You are a cover letter creator for jobs.
...
Write the text of the letter in a style suitable for the job to which the candidate is applying.
...
class Bot_Response:
    def generate(bot):
        ...
        model="gpt-3.5-turbo",
        messages=[
            {"role": "system", "content": bot.role_description},
            {"role": "user", "content": bot.user_input}
        ])

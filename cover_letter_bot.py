...
from reportlab.platypus import SimpleDocTemplate, Paragraph, Spacer  # for creating elements in PDF
from reportlab.lib.styles import getSampleStyleSheet, ParagraphStyle  # for creating styles in PDF
import tkinter as tk  # for creating graphical user interfaces
from tkinter import filedialog  # for file dialog in tkinter


openai.api_key = "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"
...
def save_cover_letter_as_pdf(cover_letter, directory, filename, sender_address, recipient_address, current_date, subject_line, name, family_name, email, phone, company, recruiter_name, recruiter_family_name):
    ...
    sender_paragraphs = [Paragraph(line, right_aligned_style) for line in [f"{name} {family_name}"] + sender_address_lines + [email, phone]]

    # Create multiple paragraphs for recipient address
    recipient_paragraphs = [Paragraph(line, left_aligned_style) for line in [company, f"{recruiter_name} {recruiter_family_name}"] + recipient_address_lines]
    ...
    cover_letter_paragraphs = [Paragraph(line, styles['Normal']) for line in cover_letter.split("\n") if line.strip() != '']

    # Build the document with the elements
    elements = sender_paragraphs + [spacer] + recipient_paragraphs + [spacer, date_paragraph, spacer, subject_paragraph, spacer] + cover_letter_paragraphs
    doc.build(elements)
...
def main():
    ...
    save_cover_letter_as_pdf(cover_letter, directory, filename, sender_address, recipient_address, current_date, subject_line, name, family_name, email, phone, company, recruiter_name,recruiter_family_name)
